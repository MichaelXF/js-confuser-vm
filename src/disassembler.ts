 ...
 *
 * Instructions are annotated `// <source location> [startPC,endPC]`, inclusive
 * of both ends, so a debugger at PC p highlights the line where
 * startPC <= p <= endPC.
 */
...
// Opcode names may contain commas (macroOpcodes), and the annotation is optional.
const INSTR_RE = /^\s*\/\/\s*\[([^\]]+)\],\s*([A-Za-z_][\w,]*)\s*(.*)$/;
...
function parseBlock(commentBlock: string): Line[] {
  const lines: Line[] = [];
  // An instruction occupies 1 slot per opcode + 1 per operand — the width of
  // the printed "[...]" array. Labels occupy none. Same math as resolveLabels().
  let pc = 0;
  ...
  switch (opName) {
    ...
    case "BUILD_OBJECT": {
      ...
      const pairCount = Math.max(
        0,
        Math.min(raw[2] ?? 0, Math.floor((raw.length - 3) / 2)),
      );
      const pairs: string[] = [];
    ...
    // Self-modifying
    case "PATCH":
      return `patch(dest=pc:${raw[1]}, src=pc:${raw[2]}..${raw[3]}, key=${raw[4]})`;
...
export function disassembleCommentBlock(commentBlock: string): string {
  ...
  for (const line of lines) {
    if (line.kind === "label") {
      ...
      const range = `[${line.instr.pc},${line.instr.pcEnd}]`;
      const outputLine = `${instrText.padEnd(50)}  // ${loc ? loc + " " : ""}${range}`;
